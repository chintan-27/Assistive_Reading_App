...
import 'package:reading_app/bookscreen.dart';
import 'package:reading_app/classifier.dart';
import 'package:reading_app/classifierbookpagemodel.dart';
import 'package:tflite_flutter_helper/tflite_flutter_helper.dart';

class BookPageScreen extends StatefulWidget {
  final List<CameraDescription>? cameras;
  const BookPageScreen({Key? key, required this.cameras}) : super(key: key);
...
class _BookPageScreenState extends State<BookPageScreen> {
  ...
  Future<String> _predict() async {
    ...
    if (category!.label == "Book" && category!.score > 0.8) {
      return "book";
    } else if (category!.label == "Page" && category!.score > 0.8) {
      return "page";
    } else {
      return "";
    }
  ...
  @override
  void initState() {
    _classifier = ClassifierBookPageModel();
                ...
                : GestureDetector(
                    onTap: () {
                      ...
                      _t = Timer.periodic(oneSec, (Timer t) async {
                        String condition = await getImage();
                        if (condition != "") {
                          t.cancel();
                          setState(() {
                            clicked = false;
                          ...
                          if (condition == "book") {
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) =>
                                    BookScreen(cameras: widget.cameras),
                              ),
                            );
                          } else if (condition == "page") {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) =>
